							...
							<tbody>
								<?PHP foreach ($listholiday as $key => $value) { ?>
									<tr class="gradeX">
										...
										<td width="5%">
											<button class="btn btn-danger fa fa-trash btn-alert" type="button" data-url="<?= site_url('holiday/delete/' . $value['set_id'] . '/' . $value['hol_id']); ?>" data-title="" data-text="ต้องการลบข้อมูล?">&nbsp;&nbsp;&nbsp;ลบข้อมูล</button>
										</td>
									</tr>
								<?PHP } ?>
							</tbody>
